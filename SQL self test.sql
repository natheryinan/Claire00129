...
--3.Write a query that returns summary statistics for avg_math_4_score by state. Make sure to 
--sort the results alphabetically by state name.

SELECT MAX(avg_math_4_score), MIN(avg_math_4_score),AVG(avg_math_4_score),state, COUNT(*)
FROM naep
GROUP BY state
ORDER BY state;
...
SELECT (MAX(avg_math_4_score)-MIN(avg_math_4_score)) AS diff, state 
FROM naep
GROUP BY state
...
WHERE year=2000
GROUP BY state
ORDER BY ROUND(AVG(avg_math_4_score),2)
LIMIT 10;
...
SELECT ROUND(AVG(avg_math_4_score),2),state AS below_average_states_y2000
FROM naep
WHERE year = 2000
GROUP BY state
Having AVG(avg_math_4_score)< 
...
SELECT avg_math_4_score, state AS score_missing_y2000
FROM naep
...
SELECT finance.state,ROUND(avg_math_4_score,2) AS avg_math_4_score,total_expenditure,finance.id
